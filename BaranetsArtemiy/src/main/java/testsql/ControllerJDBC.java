...
public class ControllerJDBC {
    ...
    @FXML
    private TableColumn<Patient,Integer> idDoctorsPatient;

    @FXML
    private TableView<Directory> directoryTable;

    @FXML
    private TableColumn<Directory,String> nameDisraseDirectory;
    @FXML
    private TableColumn<Directory,String> frequencyDirectory;
    @FXML
    private TableColumn<Directory,String> descriptionDirectory;
    @FXML
    private TableColumn<Directory,String> descriptionDepartament;

    //диагноз
    @FXML
    private TableView<Diagnosis> doagnosisTable1;

    @FXML
    private TableColumn<Diagnosis,String> nameDiagnosis1;
    @FXML
    private TableColumn<Diagnosis,String> descriptionDiagnosis1;


    @FXML
    private void NewLineDirectory(){
        Locale.setDefault(Locale.ENGLISH);
        Stage newWindow = new Stage();
        Button button = new Button("Cоздать");
        javafx.scene.control.TextField ID = new javafx.scene.control.TextField();
        ID.setText("NAMEDISRASE");
        javafx.scene.control.TextField NAME = new javafx.scene.control.TextField();
        NAME.setText("FREQUENCY");
        javafx.scene.control.TextField SURENAME = new javafx.scene.control.TextField();
        SURENAME.setText("DESCRIPTION");
        javafx.scene.control.TextField POSITION = new javafx.scene.control.TextField();
        POSITION.setText("DEPARTAMENT");
        ...
        button.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                String NAMEDISRASE = ID.getText();
                String FREQUENCY = NAME.getText();
                String DESCRIPTION = SURENAME.getText();
                String DEPARTAMENT = POSITION.getText();

                if(isNumeric(NAMEDISRASE) == true){
                    error("NAMEDISRASE field cannot contain numbers\n");
                }else NewLineDoctors();
                if(isNumeric(FREQUENCY) == true){
                    error("FREQUENCY     field cannot contain numbers\n");
                } NewLineDoctors();
                try (Connection c =
                    ...
                    PreparedStatement statement =
                            c.prepareStatement(
                                    "INSERT INTO directory" +
                                            "(NameDisease, Frequency, description, department)" +
                                            "VALUES (?, ?, ?, ?)"
                            );
//INSERT INTO `jdbs_hospital`.`doktors` (`ID`, `NAME`, `SURENAME`, `POSITION`, `SEPARATION`, `DEPARTMENT`) VALUES ('3', 'lfjlkdf', 'nlnln', 'mnlknl', 'mnlbln', '11');
                    Directory doctors = new Directory(NAMEDISRASE, FREQUENCY, DESCRIPTION, DEPARTAMENT);
                    System.out.println(doctors.toString());
                    statement.setString(1, doctors.getNAMEDISRASE());
                    statement.setString(2, doctors.getFREQUENCY());
                    statement.setString(3, doctors.getDESCRIPTION());
                    statement.setString(4, doctors.getDEPARTAMENT());
        ...
        root.getChildren().addAll(secondLabel, ID,NAME,SURENAME,POSITION, button);
    ...
    private void UpdateDirectory(){
        ...
        ObservableList<Directory> usersData = FXCollections.observableArrayList();
        if(main == false) {
            main();
        }
        nameDisraseDirectory.setCellValueFactory(new PropertyValueFactory<Directory, String>("NAMEDISRASE"));
        frequencyDirectory.setCellValueFactory(new PropertyValueFactory<Directory, String>("FREQUENCY"));
        descriptionDirectory.setCellValueFactory(new PropertyValueFactory<Directory, String>("DESCRIPTION"));
        descriptionDepartament.setCellValueFactory(new PropertyValueFactory<Directory, String>("DEPARTAMENT"));
        ...
        try (Connection c =
            ...
            List<Directory> doctors = new ArrayList<>();
            while (set.next()) {
                doctors.add(new Directory(
                        set.getString(1),
                        set.getString(2),
                        set.getString(3),
                        set.getString(4)
            ...
            statement.close();
            ObservableList<Directory> list = FXCollections.observableArrayList(doctors);
            System.out.println(list);
            try {
                usersData.addAll(doctors);
                for (Directory p : doctors) {
                    // usersData.add(p);
                    System.out.println("usersData\n" + usersData);
                    System.out.println("Получена запись: " + p);
                }
                directoryTable.setItems(usersData);
        ...
        button.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                if(isNumeric(name) == true){
                    error("Name field cannot contain numbers\n");
                }else NewLineDoctors();
                if(isNumeric(surename) == true){
                    error("Surename field cannot contain numbers\n");
                } NewLineDoctors();
...
//INSERT INTO `jdbs_hospital`.`doktors` (`ID`, `NAME`, `SURENAME`, `POSITION`, `SEPARATION`, `DEPARTMENT`) VALUES ('3', 'lfjlkdf', 'nlnln', 'mnlknl', 'mnlbln', '11');
    ...
    private void NewLinePatient(){
        ...
        button.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                if(isNumeric(diagnosis)== true){
                    error("Diagnosis field cannot contain numbers\n");
                } NewLinePatient();
    ...
    private void UpdatePatient(){
        ObservableList<Patient> usersData = FXCollections.observableArrayList();
        if(main == false) {
            main();
        }
        namePatient.setCellValueFactory(new PropertyValueFactory<Patient, String>("name"));
        surenamePatient.setCellValueFactory(new PropertyValueFactory<Patient, String>("surename"));
        diagnosisPatient.setCellValueFactory(new PropertyValueFactory<Patient, String>("diagnosis"));
        idDoctorsPatient.setCellValueFactory(new PropertyValueFactory<Patient, Integer>("idDoctors"));
        Locale.setDefault(Locale.ENGLISH);
        try (Connection c =
                     DriverManager.getConnection(url, user, pass)) {
            Statement statement = c.createStatement();
            ...
            list.get(1);
            try {
                usersData.addAll(patients);
                for (Patient p : patients) {
                    // usersData.add(p);
                    System.out.println("usersData\n" + usersData);
                    System.out.println("Получена запись: " + p);
                }
                patientTable.setItems(usersData);
    ...
    private void EditPatient(){
        ...
        try (Connection c =
            ...
            ResultSet doktors = statement.executeQuery(
                    "SELECT * FROM doktors"
            );

            List<Doctors> doctors = new ArrayList<>();
//            while (set.next()) {
//                doctors.add(new Patient(
//                        set.getInt(1),
//                        set.getString(2),
//                        set.getString(3),
//                        set.getInt(4)
    ...
    private void NewLineDiagnosis(){
        ...
        button.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                if(isNumeric(name)== true){
                    error("Name field cannot contain numbers\n");
                }else NewLineDiagnosis();
                if(isNumeric(name)== true){
                    error("DESCRIPTION field cannot contain numbers\n");
                }else NewLineDiagnosis();
...
//INSERT INTO `jdbs_hospital`.`doktors` (`ID`, `NAME`, `SURENAME`, `POSITION`, `SEPARATION`, `DEPARTMENT`) VALUES ('3', 'lfjlkdf', 'nlnln', 'mnlknl', 'mnlbln', '11');
    ...
    private void UpdateDiagnosis(){
        ObservableList<Diagnosis> usersData = FXCollections.observableArrayList();
        if(main == false) {
            main();
        }
        nameDiagnosis1.setCellValueFactory(new PropertyValueFactory<Diagnosis, String>("name"));
        descriptionDiagnosis1.setCellValueFactory(new PropertyValueFactory<Diagnosis, String>("description"));
        ...
        try (Connection c =
            ...
            statement.close();
            ObservableList<Diagnosis> list = FXCollections.observableArrayList(diagnoses);
            System.out.println(list);
            try {
                usersData.addAll(diagnoses);
                for (Diagnosis p : diagnoses) {
                    // usersData.add(p);
                    System.out.println("usersData\n" + usersData);
                    System.out.println("Получена запись: " + p);
                }
                doagnosisTable1.setItems(usersData);
